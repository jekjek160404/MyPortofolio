...
export default function Contact() {
  ...
  async function onSubmitData() {
    try {
      let res = await fetch("/api/mesage", {
        method: "POST",
        ...
        <div className="md:w-1/3">
          {!isLoading &&
            Object.keys(data.location).map((key) => {
              return (
                <ItemCard label={key} value={data.location[key]} key={key} />
              );
            })}

          {!isLoading &&
            Object.keys(data.phone).map((key) => {
          ...
          {!isLoading &&
            Object.keys(data.social).map((key) => {
              return (
                <ItemCard label={key} value={data.social[key]} key={key} />
              );
            })}
        </div>
      </div>
